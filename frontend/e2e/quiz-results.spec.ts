...
import {
  generatedQuizFixture,
  quizResultsFixture,
  validSourceText,
} from './fixtures/quiz-fixtures';

test('answers and reviews all generated results', async ({ page }) => {
  ...
  await expect(page.locator('.score-stamp strong')).toHaveText('7');
  await expect(page.locator('.result-card')).toHaveCount(10);
  await expect(page.locator('.explanation')).toHaveCount(40);
  await expect(page.locator('.evidence')).toHaveCount(10);
});
